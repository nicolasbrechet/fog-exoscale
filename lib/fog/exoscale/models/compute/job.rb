  ...
  module Compute
    class Exoscale
      class Job < Fog::Model
        ...
        def reload
          requires :id
          merge_attributes(service.query_async_job_result('jobid' => id)['queryasyncjobresultresponse'])
        end
        ...
        # so dirty
        def result
          if successful? && model = Fog::Compute::Exoscale.constants.find { |c| c.to_s.downcase == job_result.keys.first.to_s }.to_s
            collection = model.gsub(/.[A-Z]/) { |w| "#{w[0, 1]}_#{w[1, 1].downcase}" }.downcase + 's' # cheap underscorize, assume simple pluralization
            service.send(collection).new(job_result.values.first)
          else job_result
          end
        end
      end # Job
