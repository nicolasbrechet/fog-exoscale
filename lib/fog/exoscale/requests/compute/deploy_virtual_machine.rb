    ...
    class Exoscale
      ...
      class Mock
        def deploy_virtual_machine(options = {})
          ...
          unless zone_id
            fail Fog::Compute::Exoscale::BadRequest.new('Unable to execute API command deployvirtualmachine due to missing parameter zoneid')
          end
          unless zone = data[:zones][zone_id]
            fail Fog::Compute::Exoscale::BadRequest.new("Unable to execute API command deployvirtualmachine due to invalid value. Object zone(uuid: #{zone_id}) does not exist.")
          end
          zone_name = zone[:name]

          template_id = options['templateid']
          unless template = data[:images][template_id]
            fail Fog::Compute::Exoscale::BadRequest.new('Unable to execute API command deployvirtualmachine due to missing parameter templateid')
          end
          template_name = template[:name]
          template_display_text = template[:display_text]

          service_offering_id = options['serviceofferingid']
          unless service_offering = data[:flavors][service_offering_id]
            fail Fog::Compute::Exoscale::BadRequest.new('Unable to execute API command deployvirtualmachine due to missing parameter serviceofferingid')
          end
          ...
          account_name = options['account'] || data[:accounts].first[1]['name']

          domain = options['domainid'] ? data[:domains][options['domainid']] : data[:domains].first[1]
          domain_id = domain[:id]
          ...
          network_ids = Array(options['networkids']) || [data[:networks].first[1]['id']]
          networks = network_ids.map { |nid| data[:networks][nid] }
          ...
          data[:servers][identity] = virtual_machine
