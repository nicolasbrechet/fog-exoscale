...
describe Fog::Compute::Exoscale do
  ...
  describe "when authorizing rules to a new security group" do
    before do
      @new_sg = @client.create_security_group(:name => 'fog_exoscale_test_sg_rules')['createsecuritygroupresponse']['securitygroup']
      # empty security group
    end
    
    it "must have no rules" do
      @new_sg['ingressrule'].must_be_empty
      @new_sg['egressrule'].must_be_empty
    end
    ...
    describe "When adding new rules" do
      before do
        ...
        #  keeping the job id
        ingress_job_id = @client.authorize_security_group_ingress(:securityGroupName => @new_sg['name'], :cidrList => '0.0.0.0/0', :startPort => '22', :endPort => '22', :protocol => 'TCP')['authorizesecuritygroupingressresponse']['jobid']
        #  then querying the async job  
        ingress_result = @client.query_async_job_result(:jobId => ingress_job_id)['queryasyncjobresultresponse']['jobresult']
        #  and finally retrieving the rule id so we can delete it later
        @ingress_rule_id = ingress_result['securitygroup']['ingressrule'][0]['ruleid']
        
        # and same for egress rule
        egress_job_id = @client.authorize_security_group_egress(:securityGroupName => @new_sg['name'], :cidrList => '0.0.0.0/0', :startPort => '22', :endPort => '22', :protocol => 'TCP')['authorizesecuritygroupegressresponse']['jobid']
        egress_result = @client.query_async_job_result(:jobId => egress_job_id)['queryasyncjobresultresponse']['jobresult']
        @egress_rule_id = egress_result['securitygroup']['egressrule'][0]['ruleid']
        
        @new_sg = @client.list_security_groups['listsecuritygroupsresponse']['securitygroup'].select{|sg| sg['name']==@new_sg['name']}[0]
      end
      
      it "must list new rules" do
        @new_sg['ingressrule'][0]['ruleid'].must_equal @client.list_security_groups['listsecuritygroupsresponse']['securitygroup'].select{|sg| sg['name']==@new_sg['name']}[0]['ingressrule'][0]['ruleid']
        @new_sg['egressrule'][0]['ruleid'].must_equal @client.list_security_groups['listsecuritygroupsresponse']['securitygroup'].select{|sg| sg['name']==@new_sg['name']}[0]['egressrule'][0]['ruleid']
      end
